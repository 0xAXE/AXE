...
use openzeppelin_token::erc20::interface::{IERC20Dispatcher, IERC20DispatcherTrait, IERC20MetadataDispatcher, IERC20MetadataDispatcherTrait};
...
fn deploy_xp_token() -> (IXPTokenDispatcher, IERC20Dispatcher, IERC20MetadataDispatcher, IOwnableDispatcher) {
...
fn test_multiple_mints_by_owner() {
    ...
    assert!(erc20.balance_of(USER()) == mint_amount_1, "User balance should equal first mint amount");
    assert!(erc20.balance_of(OWNER()) == mint_amount_2, "Owner balance should equal second mint amount");
    assert!(erc20.total_supply() == mint_amount_1 + mint_amount_2, "Total supply should equal sum of mints");
}
